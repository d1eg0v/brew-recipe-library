...
## Coverage checklist (19 recipes)

### Beer (12 — all `category: "beer"`)
...
### Mead (4 — all `category: "mead"`, no hops / mash, `processSteps` + `additions`)

| # | Recipe | Style | BJCP | OG | FG | ABV % | Batch |
|---|--------|-------|------|----|----|-------|-------|
| 13 | Wildflower Sky traditional dry mead | Traditional Mead (dry) | M1A | 1.110 | 0.998 | 14.5 | 19 L |
| 14 | Orchard Stand semi-sweet mead | Traditional Mead (semi-sweet) | M1A | 1.090 | 1.020 | 9.2 | 19 L |
| 15 | Hedgerow blackberry melomel | Berry Melomel | M2A | 1.095 | 1.015 | 10.5 | 19 L |
| 16 | Fireside metheglin | Metheglin (spiced) | M3A | 1.085 | 1.012 | 9.6 | 19 L |

### Wine (3 — all `category: "wine"`, no hops / mash, `processSteps` + `additions`)

| # | Recipe | Style | BJCP | OG | FG | ABV % | Batch |
|---|--------|-------|------|----|----|-------|-------|
| 17 | Concord Pantry kit wine | Country Wine (Concord grape) | — | 1.090 | 0.996 | 12.2 | 4 L |
| 18 | Applefield fresh-fruit country wine | Country Wine (fresh apple) | — | 1.055 | 0.998 | 7.4 | 19 L |
| 19 | Hollowgate pyment | Pyment (honey + grape) | M4A | 1.105 | 0.998 | 14.0 | 19 L |

## Breadth summary

**By category:**
- **Beer (12)** — IPAs, pale ales, ambers, browns, stouts/porters, wheats, lagers, Belgian Dubbel/Witbier/Saison
- **Mead (4)** — dry traditional, semi-sweet traditional, fruit melomel (blackberry), spiced metheglin
...
- Fermentable `type` values: grain, sugar, honey, juice, concentrate, fruit
- Yeast attenuation ranges 73 % → 95 %

**Process / technique variety:**
- Beer: single-infusion, step, and protein-rest mashes; boil/whirlpool/dry-hop schedules
- Mead: staggered nutrient additions (SNA), bulk aging, cold-crash stabilization, backsweetening after sorbate, fruit in primary, spice steep in secondary
...
- **`fermentable.amount`:** `amountKg` for solids (grain / honey / sugar / fruit), `amountLiters` for liquids (juice / concentrate / must); both fields nullable, at least one set per row
...
- **`processStep.type`:** `primary`, `secondary`, `racking`, `stabilizing`, `aging`, `bottling`, `other` (used for mead + wine)
- **`addition.unit`:** `g`, `tsp`, `tablet`, `stick` (free-text per schema); **`addition.purpose`:** `yeast rehydration nutrient`, `yeast nutrient (organic/inorganic)`, `antimicrobial/antioxidant`, `yeast inhibitor`, `breaks down pectin for clarity`, `TA adjustment`, `structure / mouthfeel`, `spice`
...
> "boil minutes or dry-hop days"); a `notes: "dry hop, days"` marker is set on those
> rows so the loader/UI can disambiguate.

## Gaps / possible future additions

Not yet covered (candidates for a later expansion):

- **Beer:** English bitters (11A/11B), Märzen/Oktoberfest (6A), Belgian Tripel (26C), Doppelbock (9A), Kölsch (5B), sour/wild styles (23-series), hazy/NEIPA (21B)
- **Mead:** braggot (M5A), cyser (apple-honey), pyment variants beyond red-grape
